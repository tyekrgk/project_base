...
class Diyredis {
    ...
    function __construct($paServer=array(), $persist=false){
        $this->aServer = $paServer;
    ...
    private function connect(){
        if ( ! $this->connected){
            ...
            try {
                ...
                //密码验证
                if(!empty($this->aServer[2]))
                $this->auth($this->aServer[2]);
    ...
    private function errorlog($keys, $code, $msg){
        ...
        //写错误日志，die
        Lib_Logs::factory()->writeLog( 'redis', $error, true );
    }
