...
class PageModel extends Model
{
    ...
    private $classHere = 'class="now" '; //当前页链接的class样式类名，默认为here
    ...
    public function pagination($style = '1',$output=TRUE)
    {
        ...
        if($style == '1')
        {
            ...
            $page = "<div class='pagination'><ul class='page-style'>".$this->indexPage().$this->prevPage().$this->pageNumber().$this->nextPage().$this->endPage()."</ul></div>";
        }
        else if($style == '2')
        {
            //获取纯数字链接
            $page = "<div class='pagination'><ul class='page-style'>".$this->pageNumber()."</ul></div>";
        }
        else if($style == '3')
        {
            //只获取上一页下一页
            $page = "<div class='pagination'><ul class='page-style'>".$this->prevPage().$this->nextPage()."</ul></div>";
        }
        else if($style == '4')
        {
            //上一页、下一页、数字链接
    ...
    {
        if($this->currentPage > ($this->offset+1)) {
            return '<li><a href="'.$this->url.'1">1..</a></li>';
        }
    ...
    {
        if($this->pageAmount > ($this->offset+$this->currentPage) && $this->pageAmount>($this->offset*2+1)){
            return '<li><a href="'.$this->url.$this->pageAmount.'">'.$this->pageAmount.'</a></li><li><a href="javascript:;">共'.$this->totalRows.'条记录</a></li>';
        }else{
            return '<li><a href="javascript:;">共'.$this->totalRows.'条记录</a></li>';
        }
    ...
    private function prevPage()
    {
        ...
        return '<li><a  href="'.$this->url.( $this->currentPage - 1 ).'"><<</a></li>';
    }
    ...
    private function pageNumber()
    {
        ...
        if($this->pageAmount > ($this->offset * 2 + 1))
        {
            ...
            return $left.'<li><span '.$this->classHere.'>'.$this->currentPage.'</span></li>'.$right;
        }
        else
        {
            $allLink='';
            ...
            for($j = 1; $j <= $this->pageAmount; $j++)
            {
                 if($j == $this->currentPage){
                     $allLink .= '<li><span '.$this->classHere.'>'.$this->currentPage.'</span></li>';
                 }else{
                     $allLink.='<li><a  href="'.$this->url.$j.'" >'.$j.'</a></li>';
                 }
